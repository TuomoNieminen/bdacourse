...
Due to conjugacy we have the following
...
# with data x
beta_posterior <- function(a, b, x) {
  c("a" = a + sum(x), "b" = b + length(x) - sum(x))
}
beta_point_est <- function(prior_alpha, prior_beta, data) {
  th <- beta_posterior(prior_alpha, prior_beta, data)
  th["a"] / (th["a"] + th["b"])
}
...
In the case of $p( \pi \mid y)$, this corresponds to
...
We can use the beta cumulative distribution function to evaluate the probability.

$$
P(\pi < 0.2 \mid y) = F_{\pi \mid y}(0.2)
$$
...
There is quite a lot of data so reasonably uninformative or vague priors do not have a significant effect on the posterior. A very informative prior will however affect inference.
